class Solution:
    def numUniqueEmails(self, emails: List[str]) -> int:
        ...
        res=[]
        for i in emails:
            local_name=i.split('@')[0] #[0] means get the first part of the split
            local_name=local_name.replace('.','')
            local_name=local_name.split('+')[0]
            domain_name=i.split('@')[1] #[1] means get the second part of the split
            email=local_name+'@'+domain_name
            res.append(email)
            print("Local Name:",local_name,f"\nDomain Name:",domain_name,f"\nEmail:",email)
        return len(set(res))
